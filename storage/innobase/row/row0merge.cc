...
/** Set the flush observer for the transaction.
@param[in,out]	trx		transaction
@param[in]	old_table	old table
@param[in]	new_table	new table
@param[in]	index		list of indexes to be added
@param[in]	n_index		number of indexes to be added
@param[in]	stage		performance schema accounting object used
				for alter table. */
static void row_merge_set_flush_observer(
	trx_t*			trx,
	const dict_table_t*	old_table,
	const dict_table_t*	new_table,
	dict_index_t**		index,
...
{
	/* Create flush observer to flush dirty pages.
	...
	trx->set_flush_observer(new_table->space, stage);
}
...
write_buffers:
		...
		for (ulint k= 0, i = 0; i < n_index; i++, skip_sort = false) {
			...
			if (UNIV_LIKELY
				...
				/* Set the flush observer for the
				transaction when the table is not empty. */
				if (i == 0 && !file->n_rec) {
					row_merge_set_flush_observer(
						trx, old_table,
						new_table, index,
						n_index, stage);
				}
...
wait_again:
		...
		if (indexes[i]->type & DICT_FTS) {
			...
			if (dict_index_is_spatial(indexes[i])) {
				/* Avoid the MLOG_INDEX_LOAD redo log
				for spatial index. */
