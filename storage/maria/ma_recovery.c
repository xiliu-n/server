...
static int new_table(uint16 sid, const char *name, LSN lsn_of_file_id)
{
  ...
  */
  ...
  {
    /*
      This can happen if the table has been dropped and created since this
      redo entry of if the table had a bulk insert directly after create,
      in which case the create_rename_lsn ended.
    */
    tprint(tracef, ", has create_rename_lsn " LSN_FMT " more recent than"
